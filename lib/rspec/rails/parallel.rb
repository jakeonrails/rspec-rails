      ...
      class << self
        def initialize_parallel_configuration(config)
          ...
          config.parallelize_setup do |worker_number|
            ...
            # shared parent state. ActiveRecord's TestDatabases hook (which
        ...
        # All workers inherit the parent's `Rails.logger`, so without
        ...
        # leaving every worker sharing the parent's database. Set it
