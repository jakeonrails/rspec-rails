...
Custom per-worker setup is registered on the rspec configuration, same as
every other rspec-rails knob:
...
`ActiveSupport::TestCase.parallelize_setup` / `parallelize_teardown` blocks
also fire — that's how `ActiveRecord::TestDatabases` reaches rspec workers,
and it keeps Minitest-shape setup code (shared suites, in-progress
migrations from Minitest) working without modification. Users writing
net-new rspec-only code should reach for `config.parallelize_setup`.
...
in each worker) rather than pre-fork on the parent.
...
1. **Parent boots.** `rails_helper.rb` loads, triggers rspec-rails autoload,
...
Without intervention, every worker inherits the parent's `Rails.logger` and
...
heavy lifting (parent/worker protocol, DTO layer for notifications, CLI
